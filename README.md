...
Visual Studio 2022	17.10+	    Must have the Desktop development with C++ workload. 
                                Requires the v143 toolset (14.4x) to match vcpkg-built dependencies.
...
run.bat — exports VCPKG_OVERLAY_TRIPLETS as an environment variable 
(this is the one that actually matters; the CMake cache variable is ignored during manifest resolution).
...
glfwPollEvents, hot-reload config, poll shortcuts, update toasts.

Handle camera capture (right mouse or middle mouse over viewport).

Update camera (suppressed while a Blender-style transform is running).

Update tools.

Rebuild the heightmap mesh if its version counter changed.

Begin ImGui frame → draw UI (dock host, viewport, panels, toasts, status bar).

Render the scene into viewportFbo.

Blit the ImGui draw lists onto the default framebuffer.
...
Input — onMouseDown/Move/Up and onUpdate (poll-based for keyboard).
